...
def _find_credentials_path() -> str:
  """Returns the first existing Google Ads YAML path to use for auth."""
  env_path = os.environ.get("GOOGLE_ADS_CREDENTIALS")
  search_paths = [
      env_path,
      os.path.expanduser("~/google-ads.yaml"),
      os.path.join(ROOT_DIR, "google-ads.yaml"),
  ]
  for path in search_paths:
    if path and os.path.isfile(path):
      return path

  tried_paths = ", ".join(p for p in search_paths if p)
  raise FileNotFoundError(
      "Google Ads credentials YAML file is not found. "
      "Check [GOOGLE_ADS_CREDENTIALS] or place google-ads.yaml in your home "
      f"directory. Tried paths: {tried_paths}"
  )
...
def execute_gaql_core(
  ...
  This is the core implementation without the MCP tool decorator.
  """
...
def execute_gaql(
  ...
  """
  return execute_gaql_core(query, customer_id, login_customer_id)
